...
import type { Streams, SimulationResponse } from '@kbn/streams-schema';
...
export async function suggestProcessingPipeline({
  ...
  signal: AbortSignal;
  simulatePipeline(pipeline: StreamlangDSL): Promise<SimulationResponse>;
 ...
 */
function validateProcessorFailureRates(simulationResult: SimulationResponse): string[] {
  const errors: string[] = [];
...
function getUniqueDocumentErrors(simulationResult: SimulationResponse): string[] {
