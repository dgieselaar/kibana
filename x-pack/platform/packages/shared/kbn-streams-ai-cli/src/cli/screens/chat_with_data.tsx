...
export function ChatWithData({ stream }: ChatWithDataProps) {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const { back, context, state } = useAppState();

  const handleSubmit = async (input: string) => {
    // Command parsing (only slash-prefixed commands)
    if (input.startsWith('/')) {
    ...
    <Box flexDirection="column">
      <Box marginBottom={1}>
        <Text bold color="cyan">
          Chat with {stream.name}
        </Text>
        <Text dimColor> (Type /back to exit, /clear to reset, /logs to view logs)</Text>
      </Box>
