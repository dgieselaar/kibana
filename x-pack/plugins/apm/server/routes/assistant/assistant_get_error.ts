...
export async function getAssistantError({
  now,
  args,
  apmEventClient,
}: {
  now: number;
  args: t.TypeOf<typeof errorRouteRt>;
  apmEventClient: APMEventClient;
}) {
  const forceNow = new Date(now);
  const start = datemath.parse(args.start)?.valueOf()!;
  const end = datemath.parse(args.end, { forceNow })?.valueOf()!;
  ...
  const error = response.hits.hits[0]?._source;
