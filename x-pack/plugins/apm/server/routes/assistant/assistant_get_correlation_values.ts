...
export async function getAssistantCorrelationValues({
  now,
  args,
  apmEventClient,
}: {
  now: number;
  args: t.TypeOf<typeof correlationValuesRouteRt>;
  ...
  const getQueryForSet = (set: t.TypeOf<typeof setRt>) => {
    ...
    const end = datemath.parse(set.end, { forceNow })?.valueOf()!;
