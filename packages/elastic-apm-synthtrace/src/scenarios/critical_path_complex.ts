...
const spanNames = ['calculate', 'serialize', 'encode', 'doSomething', 'execute', 'write', 'readText'];
...
const scenario: Scenario<ApmFields> = async (runOptions: RunOptions) => {
  return {
    generate: ({ from, to }) => {
      ...
      const makeSubTree = (depth: number, width: number, timestamp: number, duration: number, index: number, parentPath: string, rootCausePath?: string, rootCauseDuration?: number): BaseSpan => {
        const thisPath = `${parentPath}:${depth}-${index}`;

        if (depth <= 0) {
          //console.log(`### Span name: Operation D${depth}, #children: 0`);
          return instance
            .span(`Operation D${depth}`, 'db')
            .timestamp(timestamp)
            .duration(duration);
        } else {
          ...
          const numChildren = depth > 0 ? 1 + Math.min(getRandomInt(width), getRandomInt(width), getRandomInt(width)) : 0;
          ...
          const getParallelChildren = () => {
            return [...new Array(numChildren)].map((_, i) => {
              ...
              if (rootCausePath && rootCauseDuration && rootCausePath?.startsWith(childPath)) {
                return makeSubTree(depth - 1, width, timestamp + (duration - childDuration - 1), childDuration, i, thisPath, rootCausePath, rootCauseDuration);
              } else {
                return makeSubTree(depth - 1, width, timestamp + childStart, childDuration, i, thisPath);
              }
            });
          };

          const getSequenceChildren = () => {
            var nextChildStart = Math.floor(selfTime / numChildren);
            var nextChildDuration = Math.floor(childrenDuration / (numChildren + 1)) + getRandomInt(childrenDuration / (2 * numChildren));
            var remainingChildrenDuration = childrenDuration;
            return [...new Array(numChildren)].map((_, i) => {
              ...
              nextChildDuration = (i + 1) >= (numChildren - 1) ? remainingChildrenDuration :
                Math.floor((0.7 + (0.6 * Math.random())) * (remainingChildrenDuration / (numChildren - i - 1)));
              const childPath = `${thisPath}:${depth - 1}-${i}`;
          ...
          //console.log(`### Span name: ${spanName}, #children: ${children.length}`);
          const spanName = rootCausePath && rootCausePath?.startsWith(thisPath) ? `Operation D${depth} - I${index}` : spanNames[1 + getRandomInt(spanNames.length-1)];
          return instance
      ...
      const generateFastTrace = (timestamp: number) => {
        ...
        if (slowTrace) {
          ...
          console.log(`### Root cause path: ${rootCausePath}`);
          ...
          return instance
            ...
            .children(
              makeSubTree(maxDepth, 5, timestamp + childStart, childDuration, 0, 'Tx', rootCausePath, rootCauseDuration)
            );
        } else {
          return instance
            .transaction(transactionName)
            ...
            .children(
              makeSubTree(maxDepth, 5, timestamp + childStart, childDuration, 0, 'Tx')
            );
        }
      }
